...
(select  f1.user2_id as user_id from friendship f1
where f1.user1_id=1 
union
select  f2.user1_id as user_id from friendship f2
where f2.user2_id=1)
and page_id not in 
(select distinct page_id from likes where user_id=1)

--union will remove duplicates so no need of distinct in union query
--don't forget to exclude pages recommended by 1
