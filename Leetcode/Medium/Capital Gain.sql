...
create table stocks(stock_name varchar(50),
...
Query:
select s.stock_name,(total_selling_price-total_cost_price) as capital_gain_loss
from 
(select stock_name,sum(price) as total_selling_price from stocks
where operation='Sell'
group by stock_name) s join
(select stock_name,sum(price) as total_cost_price from stocks
where operation='Buy'
group by stock_name) b on s.stock_name=b.stock_name
